...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # Aggregate the data to get the total counts of Success and Failure for all sites
        all_sites_df = spacex_df.groupby('class').size().reset_index(name='counts')
        all_sites_df['class'] = all_sites_df['class'].replace({1: 'Success', 0: 'Failure'})
        fig = px.pie(all_sites_df, values='counts', names='class', 
        ...
        fig = px.pie(site_counts, values='counts', names='class', 
